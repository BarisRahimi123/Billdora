Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'get_oauth_link') {
      const origin = req.headers.get('origin') || 'https://k6gylq35dbtl.space.minimax.io';
      ...
      const accountLinkResp = await fetch('https://api.stripe.com/v1/account_links', {
        ...
        body: new URLSearchParams({
          ...
          'refresh_url': `${origin}/settings?tab=integrations&stripe_refresh=true`,
          'return_url': `${origin}/settings?tab=integrations&stripe_connected=true`,
