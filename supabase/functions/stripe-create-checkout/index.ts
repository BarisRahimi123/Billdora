Deno.serve(async (req) => {
  ...
  try {
    ...
    const origin = req.headers.get('origin') || 'https://6bktnu61nh6j.space.minimax.io';
    const successUrl = `${origin}/invoice/${invoice_id}?payment=success`;
    const cancelUrl = `${origin}/invoice/${invoice_id}?payment=cancelled`;
