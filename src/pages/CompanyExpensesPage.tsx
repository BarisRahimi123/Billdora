    ...
    <div className="h-screen bg-neutral-50 flex flex-col overflow-hidden">
      <header className="bg-white border-b border-neutral-100 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-3 flex items-center gap-2 h-12">
          ...
          <h1 className="text-base font-semibold text-neutral-400">Company Expenses</h1>
        </div>
      </header>

      <main className="flex-1 overflow-y-auto max-w-7xl mx-auto w-full px-3 py-2 pb-20">
        {/* Summary */}
              ...
              <div key={cat.value} className="bg-white rounded-md overflow-hidden" style={{ boxShadow: 'var(--shadow-card)' }}>
                <div onClick={() => toggleCategory(cat.value)} className="px-2.5 py-1.5 flex items-center justify-between cursor-pointer hover:bg-neutral-50/50 transition-colors">
                  <div className="flex items-center gap-1.5 min-w-0 flex-1">
                    ...
                    <div className={`w-6 h-6 rounded flex items-center justify-center flex-shrink-0 ${isExpanded ? 'bg-[#476E66]/10' : 'bg-neutral-100'}`}><Icon className={`w-3 h-3 ${isExpanded ? 'text-[#476E66]' : 'text-neutral-400'}`} /></div>
                    <div className="min-w-0 flex-1">
                      <p className={`text-xs font-medium truncate leading-tight ${isExpanded ? 'text-neutral-900' : 'text-neutral-500'}`}>{cat.label}</p>
                      <p className={`text-[9px] leading-tight ${isExpanded ? 'text-neutral-500' : 'text-neutral-400'}`}>{cat.expenses.length} items{cat.total > 0 && ` • ${formatCurrency(cat.total)}/mo`}</p>
                    </div>
                  </div>
                </div>
