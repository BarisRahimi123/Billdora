...
class SupabaseService {
  ...
  // ============ PROFILES ============
  /// Get profile by Supabase Auth user ID (profile.id = auth.users.id)
  Future<Map<String, dynamic>?> getProfileBySupabaseUserId(String supabaseUserId) async {
    debugPrint('SupabaseService.getProfileBySupabaseUserId: Looking for id=$supabaseUserId');
    final response = await _client
        .from('profiles')
        .select('*, roles(*)')
  ...
  Future<Map<String, dynamic>?> getProfileByAuthId(String authUserId) async {
    return getProfileBySupabaseUserId(authUserId);
  }
  ...
  Future<Map<String, dynamic>?> getCollaboratorAccount(String authUserId) async {
    final response = await _client
        .from('collaborator_accounts')
        .select()
        .eq('id', authUserId)
    ...
    final response = await _client
        ...
        .select('''
          *,
          ...
          quotes(id, title, number, scope, total, valid_until, quote_items(*)),
          projects(id, name, description, status, due_date, client_id),
          clients:projects(client_id, clients(id, primary_name, company_name, email, phone))
        ''')
